...
export default function Contact() {
  ...
  // P-3: SweetAlert2 cargado de forma dinámica — no bloquea el bundle inicial
  const onSubmit = async (e) => {
    ...
    // Dynamic import: SweetAlert2 se descarga solo cuando se necesita (~50KB menos en bundle inicial)
    ...
    <section id="contacto" className="px-6 py-28 relative overflow-hidden">
      ...
      <div className="max-w-lg mx-auto relative z-10">
        ...
        >
          ...
          <div className="mt-4 mx-auto w-10 h-px bg-acento/60" aria-hidden="true" />
        </div>
          ...
          <div className="flex gap-3" role="list" aria-label="Redes sociales">
            {socialLinks.map((social) => (
              <a
                ...
                role="listitem"
                aria-label={`Contactar por ${social.name}${social.href !== '#' ? ' (abre en nueva pestaña)' : ' (próximamente)'}`}
        ...
        {/* Divider */}
        <div className={`anim anim-fade flex items-center gap-3 mb-5 ${socialVisible ? 'visible' : ''}`} aria-hidden="true">
        ...
        >
          <form onSubmit={onSubmit} className="space-y-5" noValidate>
            ...
            {/* A-1: htmlFor conectado con id del input */}
            <div className="flex flex-col gap-1.5">
              <label
                htmlFor="contact-name"
                className="font-barlow font-light text-xs text-principal/60 tracking-[0.18em] uppercase"
              >
                Nombre
              </label>
              <input
