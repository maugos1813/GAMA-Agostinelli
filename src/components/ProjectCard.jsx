...
export default function ProjectCard({ name, niche, description, demoUrl, image, delay = 'delay-0', isVisible = false }) {
  return (
    <div className={`anim anim-up ${delay} group glass-card rounded-3xl overflow-hidden hover:shadow-2xl hover:-translate-y-1.5 transition-all duration-300 ${isVisible ? 'visible' : ''}`}>
      ...
      <div className="p-6 flex flex-col gap-3 border-t border-white/50">
        ...
        {/* REEMPLAZAR CON URL DE DEMO CUANDO ESTÉ LISTA */}
        <a
          href={demoUrl}
          target="_blank"
        ...
        >
          Ver demo
          <span className="inline-block group-hover:translate-x-1 transition-transform duration-300 text-acento">→</span>
        </a>
      </div>
    </div>
  );
